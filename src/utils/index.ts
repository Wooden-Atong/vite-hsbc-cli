...
const modifyPackageType = (packageFilePath: string) => {
  const packageJsonData = fs.readFileSync(packageFilePath, 'utf-8');
  const packageJson = JSON.parse(packageJsonData);
  
  // 修改 "type" 属性值为 "commonjs"
  packageJson.type = 'commonjs';
  
  // 写入修改后的 package.json 文件
  const updatedPackageJsonData = JSON.stringify(packageJson, null, 2);
  fs.writeFileSync(packageFilePath, updatedPackageJsonData, 'utf-8');
}
